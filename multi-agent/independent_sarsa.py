    ...
    for episode in xrange(num_episodes):
        ...
        while not terminal(state):
            ...
            for Q, action in zip(Qpreds, pred_actions):
                newaction = selection_func(state, Q)
                Q[(state, action)] += alpha * (r_pred + gamma * Q[(newstate, newaction)]
                        - Q[(state, action)])

            new_prey_action = selection_func(state, Q)
            Qprey[(state, prey_action)] += alpha * \
                    (r_prey + gamma * Qprey[(newstate, new_prey_action)] - \
                    Q[(state, action)])
